...
class Conv2D:
    ...
    def forward(self, A_prev):
        ...
        # padding_h, padding_w = self.pad(A_prev, self.padding, 0)
            ...
            for h in range(H):
                ...
                for w in range(W):
                    ...
                    for c in range(C):
                        ...
                        weights = W[:,:,:,c]
                        biases = b[:,:,:,c]
                        Z[i, h, w, c] = self.single_step_conv(a_slice_prev, weights, biases) # hint: use self.single_step_convolve()
        return Z
    ...
    def backward(self, dZ, A_prev):
        ...
        for i in range(batch_size):
            ...
            dA_prev[i, :, :, :] = da_prev_pad[self.padding[0]:-self.padding[0], self.padding[1]:-self.padding[1], :] # hint: remove padding (trick: pad:-pad)
        grads = [dW, db]
